...
from .exceptions import DiscourseError, DiscourseServerError, DiscourseClientError
...
class DiscourseClient(object):
    ...
    def logout(self, userid, **kwargs):
        return self._post('/admin/users/{0}/log_out'.format(userid), **kwargs)
        
    def activateUserById(self, userid, **kwargs):
        return self._put('/admin/users/{0}/activate'.format(userid), **kwargs) 
        
    def getUserByUsername(self, username, **kwargs):
        return self._get('/users/{0}.json'.format(username), **kwargs) 
